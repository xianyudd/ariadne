...
from runtime import decision_engine  # noqa: E402
from runtime.classification import CLASSIFICATIONS  # noqa: E402
...
FIXTURES = SDLC / "validation" / "fixtures"
RUNTIME_DOC = SDLC / "runtime" / "README.md"
...
contract = read(CORE / "terminal-contract.md")
...
# Core owns the envelope, not a workflow's reason codes, so the reference lives
# with the runtime. Every code the engine can emit is documented there, and the
# reference names nothing the engine cannot emit.
runtime_doc = read(RUNTIME_DOC)
documented_codes = set()
for block in blocks(runtime_doc):
    for line in block.splitlines():
        token = line.strip().split()[0] if line.strip() else ""
        if re.fullmatch(r"[A-Z][A-Z_]{4,}", token) and token in decision_engine.REASON_CODES:
            documented_codes.add(token)

missing = sorted(decision_engine.REASON_CODES - documented_codes)
check(not missing, f"every reason code is documented: missing {missing}")

# Nothing reason-code-shaped in the reference is unknown to the engine.
reference = re.search(r"## Reason codes\n(.*?)(?=\n## |\Z)", runtime_doc, re.S)
check(reference is not None, "the runtime document has a reason-code reference")
assert reference is not None
declared = {
    token
    for line in reference[1].splitlines()
...
check(not unknown, f"the reference declares no code the engine cannot emit: {unknown}")
check(
...
lifecycle_doc = read(CORE / "lifecycle.md")
machine = None
...
envelope_doc = read(CORE / "decision-envelope.md")
declared_decisions = {
    line.strip()
