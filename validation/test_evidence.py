...
from runtime.classification import (  # noqa: E402
    NON_PRODUCT,
    PRODUCT_FEATURE,
    UNKNOWN,
    classify_evidence,
)
from runtime.evidence import collect_repository_evidence, workflow_only_changes  # noqa: E402
...
with make_repo(tasks=TASKS_READY, product_change=True) as fixture:
    ...
    check("src/example.rs" not in evidence.git.changed_paths, "A1 changed paths are real")
    check("src/feature.rs" in evidence.git.changed_paths, "A1 product change observed")
...
with make_repo(
    ...
    evidence = collect_repository_evidence(fixture.root)
    check(evidence.git.changed_paths == (".agent-sdlc/core/notes.md",), "A2 only workflow paths")
...
with make_repo(tasks=TASKS_READY, protected_change=True) as fixture:
    ...
    check(
        all(path.startswith(".claude/worktrees/") for path in evidence.git.protected_paths),
        "A5 protected path identified by prefix",
    )
    check(
        not any(path.startswith(".claude/worktrees/") for path in evidence.git.changed_paths),
