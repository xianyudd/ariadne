...
`CONTINUE`/`TERMINAL_*`. Markdown in `.agent-sdlc/core/` specifies these rules
for humans and agents; the tables here are what actually runs, and
`.agent-sdlc/validation/test_decision_consistency.py` fails if the two drift.
...
# Reported statuses. `.agent-sdlc/core/terminal-contract.md` fixes the terminal
...
# framework (`.agent-sdlc/core/decision-envelope.md`), so none appears here.
...
# enumerable: `.agent-sdlc/validation/test_decision_consistency.py` requires each
...
# this phase name explicitly (`.agent-sdlc/workflows/dev-merge.md`); the runtime
...
def _decide_dev_new(state: ResolvedState) -> Decision:
    ...
    `.agent-sdlc/workflows/dev-new.md` fixes this table and states explicitly
...
def _decide_dev_next(state: ResolvedState) -> Decision:
    ...
    Applicability follows `.agent-sdlc/core/lifecycle.md`: implementation happens
    ...
    (`.agent-sdlc/core/lifecycle-entity.md`), and a graph with no ready frontier
    is blocked (`.agent-sdlc/core/task-dag.md`).
    """
    if state.entity == NON_PRODUCT:
...
def _decide_dev_close(state: ResolvedState) -> Decision:
    ...
    `.agent-sdlc/workflows/dev-close.md` requires every task complete before
...
def _decide_dev_merge(state: ResolvedState) -> Decision:
    ...
    The classification table is fixed by `.agent-sdlc/core/terminal-contract.md`:
    ...
    A merge is an outward-facing mutation, so `.agent-sdlc/core/git-policy.md`
