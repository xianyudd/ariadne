...
Core owns the envelope's shape and the meaning of each field. Core does not own control flow: enforcement lives in `.agent-sdlc/runtime/`. Core also does not own any workflow's classification or reason-code vocabulary, and it never names a host tool, model, provider, or invocation syntax.
...
`decision` — required. Exactly one of the values declared in `.agent-sdlc/core/terminal-contract.md`:
