...
The successful target-file rename is tflow's persistence commit point; this product rule is documented in project policy, not inferred by generic Core.

For merge, use the repository's verified normal policy: target `main`, `--no-ff`, no squash, rebase, or amend. Require a fresh closure checkpoint and explicit user authorization where the host requires it. On conflict, stop at a Human Gate.
