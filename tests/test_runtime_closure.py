...
from ariadne import dag as dag_module  # noqa: E402
from ariadne.runtime import decision_engine, evidence as evidence_module, router as router_module  # noqa: E402
from ariadne.runtime.api import enforce, evaluate_workflow, execute_workflow  # noqa: E402
...
from ariadne.runtime.router import RouterRefused, WorkflowRouter  # noqa: E402
...
from ariadne.runtime.terminal_gate import TerminalGate  # noqa: E402
...
dag_source = (PACKAGE / "dag" / "tasks.py").read_text(encoding="utf-8")
...
check("DAG_INVALID" in dag_source, "INV-R1 the graph status word is shared")
...
for name, text in runtime_sources.items():
    if name in ("decision_engine.py", "decision.py"):
        continue
    check("reason_code=" not in text, f"INV-R1 {name} does not populate a reason code")
check(
...
api_source = (PACKAGE / "runtime" / "api.py").read_text(encoding="utf-8")
...
router_source = code_only(PACKAGE / "runtime" / "router.py")
for token in (
    "PRODUCT_FEATURE",
    "NON_PRODUCT",
    "READY_TO_CLOSE",
...
# The positive half of that claim is behavioural, not textual: the graph module holds
# compiled patterns, so the syntax it owns never appears in it as a literal. Feed it
# the syntax directly and check it decoded every part the runtime later reasons from.
owned, metadata, parse_errors = dag_module.parse_tasks(
    "- [x] T001 Add the model\n  Depends:\n- [ ] T002 Add the view\n  Depends: T001\n"
)
check(not parse_errors, "INV-R9 the graph module reads the task syntax it owns")
check([task.task_id for task in owned] == ["T001", "T002"], "INV-R9 it owns the task-ID form")
check(owned[0].done and not owned[1].done, "INV-R9 it owns the checkbox form")
check(metadata and owned[1].depends_on == ["T001"], "INV-R9 it owns the Depends: form")
...
# Ariadne's own real state and refuses the merge path.
#
# Which terminal decision it reaches is deliberately not asserted. This repository's
# state changes with every commit, and pinning a decision to it would make the suite
# a record of what the working tree happened to look like. What must hold on any real
# repository with no Feature ready to close is the property, not the value: the
# envelope is terminal, no phase is granted, and the dispatcher is never reached.
live = evaluate_workflow(ROOT, DEV_MERGE, human_gate=HUMAN_GATE_APPROVED)
check(live.decision in TERMINAL_DECISIONS, f"live repository decides {live.decision}")
