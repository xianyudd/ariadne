"""Canonical task DAG resolution for the Ariadne runtime.

The repository's tasks file remains the only task source of truth
(`contracts/task-dag.md`). This module owns the one executable implementation of
parsing, validation, and frontier calculation; `ariadne validate tasks` is a thin
rendering over it and no other runtime code re-parses that file.
