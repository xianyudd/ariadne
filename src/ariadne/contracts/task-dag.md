...
The repository's tasks file remains the only task source of truth. Do not create or maintain a parallel machine-readable copy of it.
...
```text
- [ ] T007 [US1] Implement the edit path in the editor module
  Depends: T003, T004
  Story: US1
...
For a legacy tasks file with no per-task metadata, parse checkbox state and use explicit dependency prose only when it can be proven. Report missing per-task metadata. Never treat every unchecked task as ready. An unprovable legacy frontier requires a Human Gate before implementation.
