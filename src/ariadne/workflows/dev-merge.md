...
Load `ariadne doc lifecycle`, `ariadne doc lifecycle-entity`, `ariadne doc terminal-contract`, `ariadne doc decision-envelope`, `ariadne doc state-contract`, `ariadne doc human-gates`, `ariadne doc git-policy`, `ariadne doc context-policy`, and the repository's own project policies.
...
Reason codes are defined in `ariadne doc reason-codes`.

## CLASSIFY → DECIDE

Before any Product Feature closure check, classify the current repository entity using the evidence rules in `ariadne doc lifecycle-entity`, then apply the terminal decision contract in `ariadne doc terminal-contract`:
...
/dev-merge does not apply to the current branch
```
...
Express the decision as a Decision Envelope (`ariadne doc decision-envelope`): `workflow` is this workflow's name, `phase` is `CLASSIFY`, `classification` is the entity above, `evidence` records the classification facts that were already read, and `next_legal_action` is `STOP` for every terminal branch. The envelope is the value the runtime terminal gate enforces. A host adapter reaches this workflow through the runtime, so a `TERMINAL_*` envelope produces one final report and no host dispatch: a later phase is unreachable rather than merely forbidden.
...
The default target is the repository's configured default branch. Use the repository's verified merge policy: `git merge --no-ff <feature-branch>`. Merge conflicts, target drift, data-loss risk, or unexplained changes are Human Gates. After merge, run post-merge project verification and record `CLOSED` only after it passes.
